RSpec::Matchers.define :have_no_tickets_sold do
  ...
  private
    def failure_reason(event)
      ", but had #{event.tickets_sold.size} ticket sold"
    end
end

RSpec::Matchers.define :be_sold_out do
  match { |event| event.tickets_sold.size === event.capacity }
  failure_message { |event| super() + failure_reason(event) }
  failure_message_when_negated { |account| super() + failure_reason(account) }

  private
    def failure_reason(event)
      ", but #{event.capacity - event.tickets_sold.size} tickets were not sold"
    end
end
